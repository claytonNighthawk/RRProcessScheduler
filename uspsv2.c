...
void signal_handler(int signo) {
    ...
    switch(signo) {
        case SIGUSR1:
            run = 1;
            p1putstr(STDOUT_FILENO, "Recieved signal SIGUSR1 in process ");
            p1putint(STDOUT_FILENO, getpid());
...
int main(int argc, char* argv[]) {
    ...
    for (i = 0; i < numCommands; i++) {
        p1putint(STDOUT_FILENO, getpid());
        p1putstr(STDOUT_FILENO, ": sending SIGUSR1 to pid ");
        p1putint(STDOUT_FILENO, pids[i]);
        p1putstr(STDOUT_FILENO, "\n");
        // printf("%d: sending SIGUSR1 to  pid %d\n", getpid(), pids[i]);
        kill(pids[i], SIGUSR1);
    }

    for (i = 0; i < numCommands; i++) {
        p1putint(STDOUT_FILENO, getpid());
        p1putstr(STDOUT_FILENO, ": sending SIGSTOP to pid ");
        p1putint(STDOUT_FILENO, pids[i]);
        p1putstr(STDOUT_FILENO, "\n");
        // printf("%d: sending SIGSTOP to  pid %d\n", getpid(), pids[i]);
        kill(pids[i], SIGSTOP);
    }

    for (i = 0; i < numCommands; i++) {
        p1putint(STDOUT_FILENO, getpid());
        p1putstr(STDOUT_FILENO, ": sending SIGCONT to pid ");
        p1putint(STDOUT_FILENO, pids[i]);
        p1putstr(STDOUT_FILENO, "\n");
        // printf("%d: sending SIGCONT to  pid %d\n", getpid(), pids[i]);
        kill(pids[i], SIGCONT);
    }
